...
```console
helm install my-release bitnami-mirror/jenkins
```
...
```console
helm install my-release -f values.yaml bitnami-mirror/jenkins
```
...
```console
helm upgrade jenkins bitnami-mirror/jenkins --set jenkinsPassword=$JENKINS_PASSWORD --set jenkinsHome=/bitnami/jenkins/jenkins_home
...
export JENKINS_PASSWORD=$(kubectl get secret --namespace default jenkins -o jsonpath="{.data.jenkins-password}" | base64 -d)
kubectl delete deployments.apps jenkins
helm upgrade jenkins bitnami-mirror/jenkins --set jenkinsPassword=$JENKINS_PASSWORD
```
